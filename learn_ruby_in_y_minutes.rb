...
# Ruby Notes (Combination of my own curiousity, and LearnXInYMinutes)
...
You start it with "=begin"
And you end it with "=end"
=end

# Everything is an object, there are no primitives.
puts 3.class #=> Integer
puts 3.method(:class ).class #=> Method
...
8 - 1   # Subtraction
1 * 2   # Multiplication
3 / 4   # => 0   Integer * Integer Divison
3.0 / 4 #=> 0.75 Division With Floats
3 / 4.0 #=> 0.75 Division With Floats

# Bitwise Operators
0b10010 & 0b00011 #=> 0b00010 (bitwise and)
0b10010 | 0b00011 #=> 0b10011 (bitwise or)
0b10010 ^ 0b00011 #=> 0b10001 (bitwise xor)
...
# Apart from false itself, nil is the only other 'falser' value.
# NOTE: this means even 0 or an empty string are True when converted
#       to boolean in ruby
!!nil #=> false
!!false #=> false
!!0     #=> true
...
# (Returns '1' if first item is greater, '-1' if second item is greater, '0' otherwise)
1  <=> 10 #=> -1 (1 < 10)
10 <=> 1  #=> 1  (10 > 1)
1  <=> 1  #=> 0  (1 == 1)
...
# These are meant to be used as flow-control constructs to chain statements together
# until one of them returns true or false
...
# Can only be used when using double quoted strings. Not single quotes.
...
# ...or with a range... (inclusive on both sides... unlike python which is [inclusive:exclusive])
array[1..3] #=> [2, 3, 4]
...
# Like arithmetic, [var] access is just syntactic sugar for calling a method '[]'
# on an object
array.[] 0 #=> 1
array.[] 12 #=> nil
...
# Which is roughly equialent to the following, which is unusualy to see in Ruby.
for counter in 1..5
  puts "iteration #{counter}"
end

# The `do |variable| ... end` construct above is called a `block`.
# Blocks are similar to lambdas, anonymous functions or closures in other
# programming languages. They can be passed around as objects, called, or attached as methods.
#
...
# If you still ned an index you can use 'each_with_index' and define an index
# variable.
...
begin
  ...
  puts 'RuntimError was raised now'
else
...
# Keyword arguments
# These are confusing. I should circly back to them later. all of this is confusing and bad
# content until I skip to a new part. goto: 415
def foo(bar: 'default')
  puts bar
end
...
# Note: (from me)
# You can also pass in a dynamic amount of keyword arguments as a hash, like so
def func(options = {})
  cat_name = options.fetch(:cat_name, 'no cat found :(')
  puts cat_name
end
# But you can't really add other parameters very well or i don't know how exactly

func #=> 'no cat found :('
func(cat_name: "furry") #=> "furry"
...
# In some cases, you will want to use the splat operator `*` to prompt
# destructuring
...
# You can define a class with the 'class' leyword.
class Human
  ...
  # The above functionality can be encapsulated using the attr accessor method as follows:
  attr_accessor :name
  ...
  # A class method uses self to distinguis from instance methods.
...
# A class variable is shared among the class and all of it's descendancts.
...
# Extending mdules binds their methods to the class itself.
...
ClassName.bar #=> "bar"
ClassName.qux #=> NoMethodError: undefined method `qux'
ClassName.new.bar #=> NoMethodError: undefined method `bar'
Classname.new.qux #=> "qux"
